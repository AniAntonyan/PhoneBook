...
public class ContactServiceImpl implements ContactService {
    ...
    public Contact update(Contact contact, int id) {
        if ((!Validator.checkLabel(contact.getEmailLabel(), Validation.label)) || (!Validator.checkLabel(contact.getPhoneLabel(), Validation.label)) || (!Validator.checkEmail(contact.getEmail(), Validation.regexEmail)) || (!Validator.checkPhone(contact.getPhone(), Validation.regexPhone))){
            throw  new ResponseStatusException(HttpStatus.BAD_REQUEST,"wrong Email");
        }
